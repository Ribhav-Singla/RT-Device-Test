...
export default function ({
  ...
  filterselectedEmployees: string[];
  setFilterSelectedEmployees: (filterselectedEmployees: string[]) => void;
  ...
  useEffect(() => {
    async function getData() {
      try {
        ...
        console.log("error in names: ", error);
        setLoading(false);
      }
    }
    getData();
  ...
  const handleSelect = (id: string) => {
    //@ts-ignore
    setFilterSelectedEmployees((prev) => {
      const newSelectedEmployees = prev.includes(id)
      //@ts-ignore
  ...
  return (
    <>
      <motion.div  initial={{
                ...
                animate={{
                  scale:1
                }}className="flex-col justify-center items-center max-w-[100%] absolute  left-0 z-10">
              ...
              .map((item) => (
                ...
                >
                  <input
                    ...
                    //@ts-ignore
                    checked={filterselectedEmployees.includes(item._id)}
                  />
